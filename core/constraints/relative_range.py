"""
Relative range constraint implementation.
"""
...
class RelativeRangeConstraint(Constraint):
    """
    Constraint ensuring that items in a group have prices within a relative range.
    The most expensive item can be at most max_diff% more expensive than the cheapest.
    """

    def __init__(self, group_id: str, product_ids: List[str], max_diff: float = 140):
        """
        Initialize the relative range constraint.

        Args:
            group_id: ID of the item group.
            product_ids: List of product IDs in the group.
            max_diff: Maximum allowed price difference as percentage (e.g., 140 for 40% max difference).
        """
        super().__init__()
        self.group_id = group_id
        self.product_ids = product_ids
        self.max_diff = max_diff
        self._name = f"RelativeRangeConstraint_{group_id}"
        self._priority = self.PRIORITY_HIGH

    def check_violations(self, df_products: pd.DataFrame, **kwargs) -> pd.DataFrame:
        """
        Check for violations of the relative range constraint.
        ...
        if min_price <= 0:
            logger.warning(
                f"Minimum price is zero or negative for group {self.group_id}"
            )
            return pd.DataFrame()

        # Calculate the actual price ratio
        actual_diff = (max_price / min_price) * 100

        # Check if the ratio exceeds the maximum allowed difference
        if actual_diff > self.max_diff:
            ...
            df_violations["constraint_type"] = "relative_range"
            ...
            logger.info(
                f"Found {len(df_violations)} relative range violations in group {self.group_id}. "
    ...
    def apply_to_model(
        ...
        **kwargs,
        ...
        min_price_var = pulp.LpVariable(
            name=f"min_price_{self.group_id}", lowBound=0, cat=pulp.LpContinuous
        )

        max_price_var = pulp.LpVariable(
            name=f"max_price_{self.group_id}", lowBound=0, cat=pulp.LpContinuous
        )
        ...
        for pid in product_ids:
            ...
            model += (
                ...
                f"min_price_def1_{self.group_id}_{pid}",
            )

            # max_price >= price[i] for all i
            model += (
                max_price_var >= variables[pid],
                f"max_price_def1_{self.group_id}_{pid}",
            )
        ...
        for pid in product_ids:
            ...
            binary_vars[pid] = pulp.LpVariable(
                name=f"is_min_price_{self.group_id}_{pid}", cat=pulp.LpBinary
            )
            ...
            model += (
                ...
                f"min_price_def2_{self.group_id}_{pid}_1",
            )
            model += (
                variables[pid] - min_price_var >= -big_M * (1 - binary_vars[pid]),
                f"min_price_def2_{self.group_id}_{pid}_2",
            )

        # Ensure at least one binary variable is 1
        model += (
            pulp.lpSum(binary_vars.values()) >= 1,
            f"min_price_def3_{self.group_id}",
        )
        ...
        for pid in product_ids:
            ...
            max_binary_vars[pid] = pulp.LpVariable(
                name=f"is_max_price_{self.group_id}_{pid}", cat=pulp.LpBinary
            )
            ...
            model += (
                ...
                f"max_price_def2_{self.group_id}_{pid}_1",
            )
            model += (
                variables[pid] - max_price_var >= -big_M * (1 - max_binary_vars[pid]),
                f"max_price_def2_{self.group_id}_{pid}_2",
            )

        # Ensure at least one binary variable is 1
        model += (
            pulp.lpSum(max_binary_vars.values()) >= 1,
            f"max_price_def3_{self.group_id}",
        )

        # Constraint: max_price <= min_price * (max_diff/100)
        model += (
            max_price_var <= min_price_var * (self.max_diff / 100),
            f"relative_range_{self.group_id}",
        )

        logger.debug(
            f"Added relative range constraint for group {self.group_id} with {len(product_ids)} products, "
    ...
    def get_relaxed_version(
        ...
        Returns:
            RelativeRangeConstraint: A relaxed version of this constraint
        """
        # Increase the maximum allowed difference by the relaxation factor
        # Example: If max_diff is 140% and relaxation_factor is 0.1,
        # new max_diff will be 140 * (1 + 0.1) = 154%
